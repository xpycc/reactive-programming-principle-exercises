...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val loopSubscription = Future.run() {
      ct => Future {
        // non loop trick
        def update(): Unit = {                 // will not discard, run in background thread stack
          listener.nextRequest().continue {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext(exchange => {
          ...
          removeContext()
          // get feeling of reactive programming? when request comes, it returns immediately
          // and let the http server object complete the `exchange' operations
          // instead of doing so in the `Listener' thread
        }
      )
      p.future
    }

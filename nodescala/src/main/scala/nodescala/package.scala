...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def never[T]: Future[T] = Promise[T]().future
      // `Promise.future' will never complete until one of `Promise.complete' or `Promise.success'
      //   or `Promise.failure' etc. is called
    /*Future {
      blocking {
        while (true)
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs foreach {
        f => f onComplete { case tryValue => p.tryComplete(tryValue) }
      }
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = f.value match {
      ...
      case None => throw new NoSuchElementException("Not ready")
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete( _ =>
        try p.success(cont(f)) catch {
          case NonFatal(e) => p.failure(e)
          // here if `p.success' is called, `p.failure' will not be called, vise versal
        }
      )
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete( tryVal =>
        try p.success(cont(tryVal)) catch {
          case NonFatal(e) => p.failure(e)
          // here if `p.success' is called, `p.failure' will not be called, vise versal
        }
      )
